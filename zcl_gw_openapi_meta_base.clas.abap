...
*   Set service base url (gc_uri_icf_path = 7.54, gc_root_url = 7.53 and lower), not available in 7.52
    ...
    ELSE.
      lv_service = '/sap/opu/odata4/'. "Change AGRISTO
    ENDIF.
...
*--------------------------------------------------------------------*
* Bugfix Agristo
    li_request_info->set_srv_group_cache_timestamp(
      iv_last_modified = /iwbep/cl_v4_registry_dba=>get_last_modified_of_group( me->mv_group_id ) ).
